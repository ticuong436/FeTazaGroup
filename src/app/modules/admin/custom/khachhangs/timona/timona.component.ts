...
import { KhachhangsService } from '../khachhangs.service';
 @Component({
    selector: 'app-timona',
    templateUrl: './timona.component.html',
    styleUrls: ['./timona.component.scss'],
    encapsulation: ViewEncapsulation.None,
  })
  export class TimonaComponent implements OnInit {
  ...
  LoadAll()
  {
    this._khachhangsService.GetAllDataTimona().subscribe();
    this._khachhangsService.datastimona$.subscribe((data)=>
      {
        this.Alldata = data;
  ...
  LoadDulieu() {
    ...
    this._khachhangsService.GetAllDataTimona().subscribe();
    this._khachhangsService.datastimona$
      .pipe(takeUntil(this._unsubscribeAll))
  ...
  UpdateDulieu(data) {
    data.forEach((v, k) => {
      setTimeout(() => {
        this._khachhangsService.CreateDataTimona(v).subscribe();
      this._changeDetectorRef.markForCheck();
    });
  })
  }
  ...
  LoadData(Chinhanh) {
    this.Showchitiet = false;
    this._khachhangsService.GetDataTimonaByChiNhanh(Chinhanh).subscribe();
    this._khachhangsService.datastimona$.subscribe((data)=>
      {
        this.Alldata = data;
        const Unique = [... new Set(data.map(v => v.SDT))];
  ...
  {
    row.SDT = value;
    this._khachhangsService.UpdateDataTimona(row,row.id).subscribe();
  }
  DeleteData(row)
  {
    this._khachhangsService.DeleteDataTimona(row.id).subscribe();
  }
}
